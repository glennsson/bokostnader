...
import {
  ...
  parseAreaFromLabel,
  parseAreaKvm,
  parseSalgsoppgaveText,
  sanitizeListingCosts,
} from "./salgsoppgave-parse.js";
import {
...
const FINN_AMOUNT = String.raw`\d{1,3}(?:\s\d{3})+(?:[.,]\d+)?|\d{4,}`;
...
function parseFinnVisibleKeyInfo(rawText) {
  ...
  fields.boligpris =
    pick(new RegExp(`Prisantydning\\s*(${FINN_AMOUNT})\\s*kr`, "i")) ??
    pick(new RegExp(`Pris\\s*(${FINN_AMOUNT})\\s*kr`, "i"));

  fields.kjopspris = pick(new RegExp(`Kjøpspris\\s*(${FINN_AMOUNT})\\s*kr`, "i"));
  fields.boarealKvm = parseAreaKvm(text);

  fields.totalpris = pick(new RegExp(`Totalpris\\s*(${FINN_AMOUNT})\\s*kr`, "i"));
  fields.omkostninger = pick(new RegExp(`Omkostninger\\s*(${FINN_AMOUNT})\\s*kr`, "i"));
  fields.felleskostnaderMnd = pick(new RegExp(`Felleskostnader\\s*(${FINN_AMOUNT})\\s*kr`, "i"));
  fields.fellesgjeld = pick(new RegExp(`Fellesgjeld\\s*(${FINN_AMOUNT})\\s*kr`, "i"));

  const kommunale =
    pick(new RegExp(`Kommunale avg\\.?\\s*(${FINN_AMOUNT})\\s*kr(?:\\s*per\\s*(?:år|ar))?`, "i")) ??
    pick(new RegExp(`Kommunale avg\\.?\\s*(${FINN_AMOUNT})\\s*per\\s*(?:år|ar)`, "i"));
  const eiendomsskatt = pick(new RegExp(`Eiendomsskatt\\s*(${FINN_AMOUNT})\\s*kr`, "i"));
  if (kommunale != null) {
...
function parseFinnDlKeyInfo($) {
  ...
  const setByLabel = (label, valueText) => {
    ...
    const area = parseAreaFromLabel(label, valueText);
    if (area != null) {
      if (/primær|p-rom/i.test(lower)) {
        fields.boarealKvm = area;
      } else if (fields.boarealKvm == null) {
        fields.boarealKvm = area;
      }
      return;
    }
    ...
    if (/^kjøpspris$/.test(lower)) {
      fields.kjopspris = amount;
    } else if (/prisantydning|^pris$/.test(lower)) {
    ...
    } else if (/^kommunale avg|^kommunale avgifter/.test(lower)) {
      fields.kommunaleAarlig = amount;
    } else if (/eiendomsskatt/.test(lower)) {
      if (fields.kommunaleAarlig == null) {
        fields.kommunaleAarlig = amount;
      }
...
function deepFindFinnFields(obj, depth = 0, found = {}) {
  ...
  if (found.kommunaleAarlig == null && /kommunal|eiendomsskatt/i.test(label)) {
    const value = obj.amount ?? obj.yearly ?? obj.value ?? obj.yearlyAmount;
    if (typeof value === "number" && value >= 500) {
      found.kommunaleAarlig = value;
    }
  }

  if (found.kommunaleAarlig == null && typeof obj.municipalFee === "number" && obj.municipalFee >= 500) {
    found.kommunaleAarlig = obj.municipalFee;
  }
  if (found.kommunaleAarlig == null && typeof obj.municipalTax === "number" && obj.municipalTax >= 500) {
    found.kommunaleAarlig = obj.municipalTax;
  }

  if (found.boarealKvm == null) {
    if (typeof obj.primaryRoom === "number" && obj.primaryRoom >= 10 && obj.primaryRoom <= 2000) {
      found.boarealKvm = obj.primaryRoom;
    } else if (typeof obj.usableArea === "number" && obj.usableArea >= 10 && obj.usableArea <= 2000) {
      found.boarealKvm = obj.usableArea;
    } else if (/primær|primary|boareal|bruksareal|p-rom|usable/i.test(label)) {
      const value = obj.value ?? obj.amount ?? obj.size;
      if (typeof value === "number" && value >= 10 && value <= 2000) {
        found.boarealKvm = value;
      }
    }
  }

  if (found.kjopspris == null && /^kjøpspris$/i.test(label)) {
    const value = obj.amount ?? obj.value;
    if (typeof value === "number" && value > 50000) {
      found.kjopspris = value;
    }
  }
...
export async function extractFinnListing(url) {
  ...
  let merged = sanitizeListingCosts(
    mergeExtractedData(
