...
export async function processTilstandsrapportJob(jobId) {
  ...
  try {
    ...
    const enriched = enrichParsedTilstandsrapport(parsed);

    const result = {
      found: enriched.found,
      tiltak: enriched.tiltak,
      maintenancePlan: enriched.maintenancePlan,
      sumNodvendig: enriched.sumNodvendig,
      sumUmiddelbar: enriched.sumUmiddelbar,
      sumTotal: enriched.sumTotal,
      sumAnbefalt: enriched.sumAnbefalt,
      kilde: "pdf_storage",
    };

    if (job.property_id) {
      await syncMaintenanceCosts(admin, job, enriched);
    }
